...
@with_conn
def leave(update, context, conn):
    try:
        deep_linked_space = context.args[0]
        return leave_from_space(update, context, conn, deep_linked_space=deep_linked_space)
    except IndexError:
...
def leave_from_space(update, context, conn, deep_linked_space=None):
    ...
    print("Space handle received: " + space_handle)
